...
- UX Rescue Phase 1A يعيد بناء أساس مساحة عمل الملاحظات على `/notes/workspace` (Master-detail، إجراءات Assign/StartWork/RequestVerification/RejectVerification/VerifyClosure/Reopen/Cancel داخل الـWorkspace، تنقّل سابق/تالي، توافق `/notes` و`/notes/:id` القديمة عبر `VITE_OBSERVATION_WORKSPACE_V2`). تنفيذ جزئي من Issue #143 (Vertical Slice أولى، ليست كل عمليات الملاحظات)، ولا يغلق #143 أو #11 أو #18 أو #19 — [`docs/ux-rescue/phase1a-observation-completion-report.md`](docs/ux-rescue/phase1a-observation-completion-report.md).
- UX Rescue Phase 1A.1 يصحح PR #149 بحيث لا تُعرض تفاصيل الملاحظة كـpopup أو `dialog` من Facility Workspace. ملاحظات السجن اليومية تنتقل إلى `/notes/workspace?facilityId=...&noteId=...&source=facility:...` وتُدار داخل Detail Pane في الصفحة نفسها، مع بقاء Legacy fallback وFeature Flag وبدون بدء Phase 1B — [`docs/ux-rescue/phase1a1-observation-completion-report.md`](docs/ux-rescue/phase1a1-observation-completion-report.md).
