...
| Forms.ManageTemplates | `Forms.ManageTemplates` (إدارة/إنشاء قوالب) و`Forms.View` (معاينة/سرد القوالب، بما فيها نقطة المعاينة الجديدة `GET /api/v1/form-templates/{id}/schema`) | استخدام قالب لبدء نموذج يتطلب `Forms.ManageTemplates` أيضًا — `CreateFormFromTemplateAsync` يفحص كلًا من `Forms.ManageTemplates` و`Forms.Create` معًا قبل الإنشاء (غير معدَّل في هذه المرحلة)؛ لا يكفي `Forms.Create` وحدها |
