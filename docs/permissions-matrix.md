...
تنطبق على سجل المخاطر، المصفوفات، التقييمات، الضوابط، خطط ومعالجة المخاطر، المراجعات، الروابط النمطية، والاستيراد/المصالحة. لا تمتد من `Workspaces.ViewFacility`. دور جديد `RiskOfficer` (ضابط مخاطر) يحمل حزمة إدارة على مستوى السجن (18 صلاحية) — **وليست** "الحزمة الكاملة": يستثني عمدًا 7 صلاحيات اعتماد (`ReviewAssessment`, `ApproveAssessment`, `VerifyTreatmentActions`, `ApproveAcceptance`, `ApproveClosure`, `Export`, `ApproveMatrices`) لأن هذه محكومة بفصل المهام (four-eyes) لا بصلاحية منفصلة قابلة لمنشئ العملية نفسه. `Auditor` غير مذكور في الجدول أدناه لأنه لا يحصل إلا على `Risks.ViewSummary` — نفس حزمة `RegionalDirector`/`HeadquartersExecutive` (انظر السطر الأخير من هذا القسم).
