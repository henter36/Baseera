...
`RiskScoringEngine.SelectRatingBand` يرتّب النطاقات حسب `MinimumScore` ويختار أول نطاق تحقق فيه `الدرجة ≥ الحد الأدنى` مع اعتبار الحد الأعلى **غير شامل** لكل نطاق عدا الأخير. النطاق الأخير وحده حده الأعلى **شامل أيضًا** (`Minimum` و`Maximum` كلاهما inclusive). **أي درجة تتجاوز `MaximumScore` للنطاق الأخير تُرفض صراحة** برسالة عربية واضحة (`InvalidOperationException`) بدل أن تُحسب تلقائيًا ضمن النطاق الأعلى — الحد الأعلى للنطاق الأخير شامل، لكن أي درجة تتجاوز الحد الأقصى النهائي تعتبر خارج تعريف المصفوفة وتؤدي إلى رفض التقييم برسالة واضحة، لأن المصفوفة يجب أن تُصمَّم لتغطية كل درجة يمكن أن تُنتجها مستوياتها. هذا العقد الحدودي مقصود وليس تبسيطًا: صيغة `LikelihoodTimesWeightedImpact` تُنتج درجات كسرية (مثال: احتمالية 3 × متوسط أثر موزون 2.33 = 6.99)، فلا يصح افتراض حدود صحيحة (integer) عند "+1" بين النطاقات كما في الصيغة الأولى القديمة — ذلك كان يترك فجوة فعلية (مثل 5.5 بين نطاقين [1,5] و[6,12]) ترمي استثناءً رغم أن المصفوفة "صالحة" ظاهريًا. بما أن `RiskMatrixValidation.ValidateRatingBands` يفرض تغطية متلامسة بلا فجوة (`min(التالي) = max(السابق)`) عند اعتماد المصفوفة، فإن عدم وجود نطاق مطابق ضمن المدى المعرَّف يعني خللًا في إعداد المصفوفة نفسها (`InvalidOperationException` دفاعية أيضًا).
