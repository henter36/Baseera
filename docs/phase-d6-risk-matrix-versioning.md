...
* النطاقات متلامسة تمامًا بلا فجوة ولا تداخل: `min(التالي) = max(السابق)` (وليس `+1`). هذا مقصود: صيغة `LikelihoodTimesWeightedImpact` تُنتج درجات كسرية (مثلاً 6.99)، فلا يصح افتراض حدود صحيحة (integer) بين النطاقات.

**عقد الحدود عند الاختيار الفعلي (`RiskScoringEngine.SelectRatingBand`)**:

* كل نطاق عدا الأخير: الحد الأدنى **شامل**، الحد الأعلى **غير شامل** (exclusive) — نقطة التماس تنتمي دائمًا للنطاق الأعلى.
* النطاق الأخير: الحد الأدنى **شامل**، والحد الأعلى **أيضًا شامل** (inclusive) — خلافًا لبقية النطاقات.
* أي درجة **تتجاوز** الحد الأعلى للنطاق الأخير تُرفض صراحة (`InvalidOperationException`) بدل أن تُحسب ضمن النطاق الأعلى تلقائيًا. هذا يعني أن المصفوفة يجب أن تُصمَّم لتغطية كامل المدى النظري الذي يمكن أن تنتجه مستويات الاحتمالية والأثر المعرَّفة فيها؛ إن لم تُغطِّه فهذا خلل في إعداد المصفوفة يجب أن يظهر بوضوح وقت التقييم لا أن يُخفى.
