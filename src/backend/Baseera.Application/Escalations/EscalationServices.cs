    ...
    TimeProvider timeProvider) : IEscalationProcessor
{
    ...
    private async Task<List<EscalationCandidate>> LoadCandidatesAsync(EscalationPolicy policy, EscalationRule rule, DateTimeOffset now, int batchSize, CancellationToken cancellationToken)
    {
        ...
        var actionCandidates = await caQuery.Take(Math.Clamp(batchSize, 1, 1000)).Select(x => new EscalationCandidate(
            ...
            x.Action.DueAtUtc.GetValueOrDefault(),
            x.Note.ScopeType,
            x.Note.RegionId,
            x.Note.FacilityId,
    ...
    private async Task<List<Guid>> ResolveRecipientsAsync(EscalationRule rule, EscalationCandidate candidate, CancellationToken cancellationToken)
    {
        ...
        query = rule.RecipientStrategy switch
        {
            ...
            EscalationRecipientStrategy.SpecificRoleInTargetScope when !string.IsNullOrWhiteSpace(rule.RecipientRoleCode) => UsersInRole(query, rule.RecipientRoleCode),
